...
function hidePreloader() {
    setTimeout(() => {
        ...
        if (preloader) {
            preloader.classList.add("not-act");
...
function getLocalTime() {
    ...
    return date.toUTCString() + ' ' + getUTCOffset();
}
...
async function detectHardwareAndEnvironment() {
    ...
    try {
        ...
        // User-Agent Data API
        let browser = parsedData.browser;
        if (navigator.userAgentData && navigator.userAgentData.brands) {
            browser = navigator.userAgentData.brands.slice(0, 2)
                .map(brand => `${brand.brand} (ver.${brand.version})`).join(' / ');
        }
        ...
        if (!navigator.onLine) {
            ...
            offlineSt.innerHTML = `<h1 style="margin-bottom:20px">OFFLINE</h1><p>You are currently offline, so some features of this page may be unavailable. <br/>Please check your internet connection and refresh the page to continue.</p>`;
            ...
            try {
                ...
                return {
                    html: `
                        ...
                        <div class="box-nav"><div><strong>IP address:</strong></div> <div>${data.ip}</div></div>
                        <div class="box-nav"><div><strong>Network:</strong></div> <div>${data.network}</div></div>
                        <div class="box-nav"><div><strong>Version:</strong></div> <div>${data.version}</div></div>
                        <div class="box-nav"><div><strong>Organization:</strong></div> <div>${data.asn + ', ' + data.org}</div></div>
                        <div class="box-nav"><div><strong>Languages:</strong></div> <div>${data.languages}</div></div>
                        <div class="box-nav"><div><strong>Country:</strong></div> <div>${data.country_name}</div></div>
                        <div class="box-nav"><div><strong>Region:</strong></div> <div>${data.region}</div></div>
                        <div class="box-nav"><div><strong>City:</strong></div> <div>${data.city}</div></div>
                        <div class="box-nav"><div><strong>Postal:</strong></div> <div>${data.postal}</div></div>
                        <div class="box-nav"><div><strong>Country Calling Code:</strong></div> <div>${data.country_calling_code}</div></div>
                        <div class="box-nav"><div><strong>Currency:</strong></div> <div>${data.currency}</div></div>
                        <div class="box-nav"><div><strong>Time Zone:</strong></div> <div>${data.timezone}</div></div>
                        <div class="box-nav"><div><strong>Local Time:</strong></div> <div>${addUTCOffset(data.utc_offset).toUTCString() + ' ' + formatToUTC(data.utc_offset)}</div></div>
                    `
                };
            } catch (error) {
                // Fallback to ipify
                try {
                    ...
                    if (!response.ok) throw new Error('ipify.org: ' + response.status);
                    const data = await response.json();
                    return {
                        html: `
                            <h2>IP Information</h2>
                    ...
                    return { html: `<h2>IP Information</h2><div class="box-nav"><div><strong>IP address:</strong></div> <div>Unavailable</div></div>` };
                }
            }
        }
        ...
        infoDiv.innerHTML = `
            <div class="box">
                ...
                <p class="ip">${ipInfo.html}</p>

                <h2>Time Information</h2>
                <div class="box-nav"><div><strong>Time Zone:</strong></div> <div>${timeZone}</div></div>
                <div class="box-nav"><div><strong>Local Time:</strong></div> <div>${getLocalTime()}</div></div>

                <h2>Hardware Information</h2>
                <div class="box-nav"><div><strong>User Agent:</strong></div> <div>${userAgent}</div></div>
                <div class="box-nav"><div><strong>Browser:</strong></div> <div>${browser}</div></div>
                <div class="box-nav"><div><strong>Platform:</strong></div> <div>${parsedData.device + " / " + parsedData.os}</div></div>
                <div class="box-nav"><div><strong>Languages:</strong></div> <div>${languages}</div></div>
                <div class="box-nav"><div><strong>Logical Processors:</strong></div> <div>${logicalProcessors}</div></div>
                <div class="box-nav"><div><strong>Memory:</strong></div> <div>${memory}</div></div>
                <div class="box-nav"><div><strong>GPU Vendor:</strong></div> <div>${gpuVendor}</div></div>
                <div class="box-nav"><div><strong>GPU Renderer:</strong></div> <div>${gpuInfo}</div></div>
                <div class="box-nav"><div><strong>Performance Test:</strong></div> <div>${performanceTest}</div></div>
                <div class="box-nav"><div><strong>Online Status:</strong></div> <div>${navigator.onLine ? 'Online' : 'Offline'}</div></div>

                <h2>Screen Information</h2>
                <div class="box-nav"><div><strong>Resolution:</strong></div> <div>${screenWidth} x ${screenHeight}</div></div>
                <div class="box-nav"><div><strong>Color Depth:</strong></div> <div>${colorDepth}</div></div>
                <div class="box-nav"><div><strong>Pixel Ratio:</strong></div> <div>${pixelRatio}</div></div>
                <div class="box-nav"><div><strong>Orientation:</strong></div> <div>${orientation}</div></div>

                <h2>Browser Features</h2>
                <div class="box-nav"><div><strong>WebRTC:</strong></div> <div>${features.webRTC ? 'Yes' : 'No'}</div></div>
                <div class="box-nav"><div><strong>WebGL:</strong></div> <div>${features.webGL ? 'Yes' : 'No'}</div></div>
                <div class="box-nav"><div><strong>Service Worker:</strong></div> <div>${features.serviceWorker ? 'Yes' : 'No'}</div></div>
                <div class="box-nav"><div><strong>Push Notifications:</strong></div> <div>${features.pushNotifications ? 'Yes' : 'No'}</div></div>
                <div class="box-nav"><div><strong>Local Storage:</strong></div> <div>${features.localStorage ? 'Yes' : 'No'}</div></div>
                <div class="box-nav"><div><strong>Session Storage:</strong></div> <div>${features.sessionStorage ? 'Yes' : 'No'}</div></div>
                <div class="box-nav"><div><strong>Geolocation:</strong></div> <div>${navigator.geolocation ? 'Supported' : 'Not Supported'}</div></div>

                <h2>Network Information</h2>
                <div class="box-nav"><div><strong>Network Type:</strong></div> <div>${networkType}</div></div>
                <div class="box-nav"><div><strong>Network Speed:</strong></div> <div>${networkSpeed}</div></div>

                <h2>Battery Information</h2>
                <div class="box-nav"><div><strong>Battery Level:</strong></div> <div>${batteryLevel}</div></div>
                <div class="box-nav"><div><strong>Charging:</strong></div> <div>${isCharging}</div></div>
		...
		<div class="box-nav links"><div><strong>Webbrowsertools:</strong></div> <div><a href="https://webbrowsertools.com/" target="_balnk">webbrowsertools.com</a></div></div>
